...
Item insert_seed_item(pqxx::connection &conn,
                      std::optional<std::string> physical_label,
                      const std::string &category,
                      const std::string &sub_category,
                      const std::string &contents) {
  InsertInput in{physical_label, category, sub_category, contents};
...
void test_search_items_returns_hits_for_term_in_searchable_fields() {
  ...
  const std::vector<SearchHit> results = state::search_items(conn, "santa", 20);

  assert_true(!results.empty(), "expected at least one search hit for term santa");

  const auto it = std::find_if(results.begin(), results.end(), [&](const SearchHit &hit) {
...
void test_search_items_respects_limit() {
  ...
  const std::vector<SearchHit> results = state::search_items(conn, "santa", 2);
...
void test_search_items_boosts_contents_prefix_match() {
  ...
  const std::vector<SearchHit> results = state::search_items(conn, "santa", 10);
